...
export const updateNewNFT = async ({ tokenid, store, author, worknumber, category, name, description, priceEth, auction, auctiontime, totalsupply, instock, physical, file, thumbnail }: CreateNFTParams): Promise<ApiResponse> => {
    try {
        await initialize()
        const { arts } = global
...
export const updateCampaign = async ({
    ...
    lasttime,
    file,
}: CampaignParams): Promise<ApiResponse> => {
    try {
        await initialize()
        const data: any = {
            id: 1,
...
export const getArtLiked = async (
    uid: number,
    id: number
): Promise<boolean> => {
    try {
...
export const setArtLiked = async (
    uid: number,
    id: number
): Promise<boolean> => {
    try {
        ...
        if (art) {
            ...
            if (row === null) {
                art.likes++
                await Artlikes.insert({ id: refid, uid })
...
export const getOffersByTokenId = async ( tokenid: number, uid: number ): Promise<Array<OfferWithArt>> => {
    return await getOffers(uid, { tokenid, status: 1 }, 100)
}

export const getOffersByUID = async (
    uid: number
): Promise<Array<OfferWithArt>> => {
    return await getOffers(uid, { uid }, 100)
}
    ...
    try {
        ...
        if (global.lastCheckTime !== 0) {
            ...
            const rows = await Arts.find({
                drop: 1,
                auction: 1,
                auctiontime: { $lt: created },
            })
            if (rows) {
                for (const v of rows) {
                    const won = await Offers.findOne(
                        { tokenid: v.id },
                        { price: -1 }
                    )
                    if (won) {
                        const txid = await callBySigner(
                            conf.storefront,
                            abiStorefront,
                            'setAuctionWinner',
                            v.id,
                            toHex(won.price * 1e12),
                            v.quantity,
                            toHex(won.price * v.quantity * 1e12),
                            won.buyer
                        )
                        if (txid) {
                            await Txs.insert({
                        ...
                        await Offers.delete({ tokenid: v.id })
                    }
                }
                await checktxs()
            }
